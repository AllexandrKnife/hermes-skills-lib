...
- ОБЁРТКА hermes (основной механизм): бинарь hermes в PATH заменяется
  скриптом, который добавляет -s ... при интерактивном запуске. Оригинал
  сохраняется рядом как hermes.real. Работает в любом shell, не зависит от
  .bashrc. Служебные подкоманды (gateway, cron, mcp, serve, update...) — без -s.
  Запись обёртки — через временный файл + mv (не cat > симлинк: тот пишет
  в цель симлинка и портит оригинал — баг 16.08.2026, есть самовосстановление).
