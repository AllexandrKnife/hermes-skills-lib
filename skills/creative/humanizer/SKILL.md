---
name: humanizer
description: "Use when Humanize text: strip AI-isms and add real"
critic_status: done
version: 2.5.1
...
4. Ensure the revised text:
   ...
   - Adds specifics only from the source text or a verified source; never invents facts, names, dates, or studies to make the text livelier (the concrete-attribution fix in pattern 2 means real sources, not plausible ones)
...
Original author: Siqi Chen ([@blader](https://github.com/blader)). Original repo: https://github.com/blader/humanizer (version 2.5.1). Ported to Hermes Agent with Hermes-native tool references (`read_file`, `patch`, `write_file`) and guidance for when to load the skill. The original 29 patterns come from the source, and the before/after examples (including the full worked example) are kept as demonstrations. Patterns 30-34 and the "marketing and blog clichés" list added to pattern 7 are Hermes additions and are not part of the upstream source. The skill's own instructional prose has also been lightly edited to follow its own guidance (for example, removing em dashes and negative parallelism from the narration) so the skill models the writing it asks for. Pattern 19's example was fixed during a TRIZ audit (15.08.2026): the Before/After quotes were identical after porting, typographic quotes were restored in Before. Russian AI-marker dictionary added as `references/ru-markers.md` (Hermes addition). Original MIT license preserved in the `LICENSE` file alongside this `SKILL.md`.

Key insight from Wikipedia: "LLMs use statistical algorithms to guess what should come next. The result tends toward the most statistically likely result that applies to the widest variety of cases."

## Pitfalls

- Паттерны 1-6, 7, 20-29 — англо-специфичны: для русского текста применять `references/ru-markers.md`, а не переводить английские словари дословно (русские AI-клише другие: «в современном мире», «стоит отметить», «данный»).
- Не фабриковать конкретику: детали и атрибуции — только из источника (правило Process). «Оживление» выдуманными фактами превращает правку во вред.
- Типографские кавычки — AI-маркер в английском тексте, но «ёлочки» («») и «лапки» („“) — норма русской типографики: не вычищать их из русского текста.
- Если образец голоса дан (Voice Calibration) — следовать ему; не подменять «универсально-естественным» стилем из PERSONALITY AND SOUL.
- Пропуск финального anti-AI pass (шаги 6-9 Process) возвращает половину паттернов — самопроверка обязательна, не опциональна.
