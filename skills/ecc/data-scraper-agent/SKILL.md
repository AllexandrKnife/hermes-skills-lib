...
version: 1.0.0
description: "Use when Build a fully automated AI-powered data"
critic_status: done
metadata:
  origin: community
---
...
| **0 items from a source that used to return N** | The site changed its HTML/API — the selector died, the scraper silently returned empty, and an empty run looks like "nothing new today" instead of a breakage | Treat a drop to 0 as an alert "check source schema", not a quiet success; add a per-source expected-volume floor (config) and log a warning when it's breached |
