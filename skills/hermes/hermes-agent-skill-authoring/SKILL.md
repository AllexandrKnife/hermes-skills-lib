...
description: "Use when Author in-repo SKILL.md files: frontmatter"
critic_status: done
version: 1.1.0
author: Hermes Agent
license: MIT
...
9. **One skill, one job (дробление).** If a SKILL.md covers two unrelated procedures, split it into two skills or push one into references. A loaded skill pays for its whole context on every trigger; two jobs in one file means half the content is always dead weight.
...
4. **Validate locally**:
   ...
   ```
5. **Прогнать через document-critic** (обязательно для каждого нового/изменённого скилла):
   - Слои 1-3 + 7: фактология (названия, тарифы, габариты, проценты), источники, противоречия между разделами, диапазоны без базы
   - Исправить ВСЕ замечания одним батчем, перепроверить (слои 1-3)
   - Критика внутренняя, в SKILL.md не включается
   - **Установить `critic_status: done` в frontmatter** — жёсткий gate в skill_manager_tool.py блокирует create/edit/patch без него (ошибка «Critic gate»)
...
- [ ] Пропущен через document-critic (слои 1-3+7), замечания исправлены одним батчем
